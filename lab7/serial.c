...
{ 
  ...
  *************************************/
  ...
  tty->inhead = (tty->inhead + 1) % INBUFLEN;
  printf("2");
...
int do_tx(struct stty *tty)
{
	...
	************************************************************/
	...
	printf("%c", tty->outbuf[tty->outtail]);
	tty->outtail++;
